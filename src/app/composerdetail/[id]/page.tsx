...
export default function ComposerDetail({ params }) {
    ...
    useEffect(() => {
        ...
        axios.get(`/music?id_composer=${id}`)
            .then((response: any) => {
                if (response && response.result.data) {
    ...
    return (
        <div className={style.container}>
            <h1>{composer.name}</h1>
            {composer.url_cover && <img src={composer.url_cover} alt={composer.name} />}
            <h2>Bài hát của {composer.name}</h2>
            <ul>
                {musicList.length > 0 ? (
                    musicList.map((music) => (
                        <li key={music.id_music}>
                            {music.url_cover && <img src={music.url_cover} alt={music.name} />}
                            <p>{music.name}</p>
                        </li>
                    ))
                ) : (
